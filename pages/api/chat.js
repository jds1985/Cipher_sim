...
async function recentTexts(n = 8) {
  const snap = await adminDB.collection("cipher_memory")
...
async function recentInsights(n = 5) {
  const snap = await adminDB.collection("cipher_insights")
    .orderBy("timestamp", "desc")
    .limit(n)
    .get();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (command && command !== "chronicle") message = message.trim().replace(/^\/\w+\s*/i, "");
