...
import { db } from "../firebaseConfig"; // make sure firebaseConfig.js is in root
import {
  collection,
  query,
  orderBy,
...
export default function MemoryField() {
  ...
  // 🔁 Load and persist chosen session
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      // collect session IDs for dropdown
  ...
  const filteredAsc = useMemo(() => {
    ...
    const keep = memories.filter((m) => {
      const bySession = (m.sessionId || "default") === sessionId;
