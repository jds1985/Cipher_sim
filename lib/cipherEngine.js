  ...
  try {
    ...
    if (typeof window !== "undefined" && !window.pipeline && !window.transformers) {
      await new Promise((resolve, reject) => {
        ...
        // Using the official browser bundle endpoint
    ...
    // 🎯 Properly trace and unpack the pipeline method from the official v3 global object name
    const hfModule = typeof window !== "undefined" ? window.transformers : null;
    const pipeline = hfModule ? hfModule.pipeline : null;

    if (!pipeline) {
      throw new Error("Core pipeline method could not be extracted from global transformers module.");
    }
