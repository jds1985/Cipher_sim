 ...
 * Loads the core library engine directly from an optimized client-side script injection.
 */
export async function initializeCipher(onProgressCallback) {
  ...
  try {
    // 🌐 Dynamic Client-Side Script Injection
    // Pulls the native browser-optimized build directly, keeping Vercel completely out of it
    if (!window.pipeline) {
      await new Promise((resolve, reject) => {
        const script = document.createElement("script");
        script.src = "https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.0.1";
        script.type = "module";
    ...
    // 🔐 Mask the string in a variable to blindfold the Next.js static compiler build analyzer
    const moduleName = '@huggingface/transformers';
    const { pipeline } = await import(/* webpackIgnore: true */ moduleName).catch(() => {
      // Fallback direct reference to the global window variable instantiated by the CDN script
      return { pipeline: window.pipeline };
    });

    if (!pipeline) {
      throw new Error("Could not instantiate HuggingFace pipeline from client substrate.");
    }
