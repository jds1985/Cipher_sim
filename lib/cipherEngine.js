...
export async function bootCipherEngine() {
    ...
    try {
        // FIXING THE 404: Pointing to the native base architecture file mapping name
        const modelUrl = 'https://huggingface.co/Jds1985/cipher-substrate-weights/resolve/main/model.onnx';
        ...
        // Fallback catch if your main repo file is named model_quantized.onnx instead
        if (modelResponse.status === 404) {
            console.log("🔄 Base model map not found, attempting alternative quantized name path...");
            const altModelUrl = 'https://huggingface.co/Jds1985/cipher-substrate-weights/resolve/main/model_quantized.onnx';
            const altModelResponse = await fetch(altModelUrl);
            
            if (!altModelResponse.ok || !weightsResponse.ok) {
                throw new Error(`Cloud asset fetch failed. Status: Model(${altModelResponse.status}), Weights(${weightsResponse.status})`);
            }
            
            const modelBuffer = await altModelResponse.arrayBuffer();
