...
// Fetch helper with direct chunk-by-chunk download reporting
...
// ==========================================
// AUTOMATED ZERO-STITCH BOOT ENGINE
// ==========================================
export async function bootCipherEngine({ onProgress = () => {} } = {}) {
    ...
    try {
        ...
        if (!limitsCleared) {
            console.log("📥 Substrate database empty. Commencing direct download stream...");
            for (let i = 0; i < shardUrls.length; i++) {
                const shardArray = await fetchWithProgress(shardUrls[i], `Shard ${i+1}`, (shardPercent) => {
                    if (typeof onProgress === 'function') {
                        onProgress({ 
                            ...
                            msg: `Syncing Shard Part ${i + 1}/4... ${shardPercent}%`,
                ...
                console.log(`💾 shard_${i} safely committed to disk database.`);
            }
        }

        if (typeof onProgress === 'function') onProgress({ stage: "COMPILING", pct: 85, msg: "Mapping system architecture files..." });
        
        // SAFE HYDRATION: Read shards individually from storage right into the option mapping array
        console.log("🧠 Pulling shard arrays from local database partition...");
        const externalBuffers = [];
        for (let i = 0; i < shardUrls.length; i++) {
            const shardData = await getStoredShard(db, `shard_${i}`);
            // Push individual buffers directly without combining them into a giant variable
            externalBuffers.push(shardData);
        }

        if (typeof onProgress === 'function') onProgress({ stage: "COMPILING", pct: 95, msg: "Mounting hardware configurations..." });
        const modelResponse = await fetch(modelUrl);
        const modelBuffer = await modelResponse.arrayBuffer();

        console.log("🚀 Initializing WebGPU runtime matrix natively via separate memory buffers...");
        
        // Feed the array of separate shard buffers natively to ONNX Runtime to avoid RAM allocation limits
        session = await ort.InferenceSession.create(new Uint8Array(modelBuffer), {
            executionProviders: ['webgpu'],
            externalWeightsBuffers: externalBuffers // PASSING THE SHARDS INDEPENDENTLY
        });
