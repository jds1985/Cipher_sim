...
function getGlobalOrt() {
    if (typeof window !== 'undefined' && window.ort) {
        ...
        globalOrt.env.wasm.proxy = true; // Safe background worker sandbox isolation
        return globalOrt;
    }
    return null;
}
...
// ==========================================
// 1. ENGINE COLD BOOT (Official Native ONNX Shard Stream)
// ==========================================
export async function bootCipherEngine() {
    if (session) return session;
    console.log("🦅 Initializing decentralized substrate via official ONNX native shard mapper...");
    ...
    try {
        ...
        // Standardized ONNX Runtime Web API for loading multi-gigabyte models across separate shards
        session = await ort.InferenceSession.create(modelUrl, {
            executionProviders: ['webgpu'],
            // FIXED: Removed the 'preferredOutputLocation' flag which is illegal when running over a web proxy worker thread
            externalData: [
                {
