...
export async function initializeCipher(onProgressCallback) {
  ...
  try {
    ...
    // Access the global execution variables instantiated by the browser script
    const { pipeline } = await import('@huggingface/transformers').catch(() => {
      // Fallback direct reference if standard module scoping isolates the window assignment
